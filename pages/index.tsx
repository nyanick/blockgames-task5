...
const Home: NextPage = () => {
  ...
  const [contractYT, setContractYT] = React.useState<any>(null);
  const [contractYTS, setContractYTS] = React.useState<any>(null);
  ...
  const [YTtoETH, setYTtoETH] = React.useState<any>(null);
  const [myYT, setMyYT] = React.useState<number>(0);
  const [transactions, setTransactions] = React.useState<any[]>([]);

  const [stakeBal, setStakeBal] = React.useState<any>(null);
  const [hasStake, setHasStake] = React.useState<any>(null);
  const [timeOfStaking,setTimeOfStaking] = React.useState<any>(null);
  const [numberOfStakers,setNumberOfStakers] = React.useState<any>(null);
  ...
  const onCloseS = () => setIsOpenS(false);

  const [isOpenW, setIsOpenW] = React.useState<boolean>(false);
  const onCloseW = () => setIsOpenW(false);
  ...
  const handleUseEffect = async () => {
    ...
    load().then((e) => {
      ...
      setContractYT(e.contractYT);
      setContractYTS(e.contractYTS);
      setTokensSold(e.tokensSold);
      setTransactionCount(e.transactionCount);
      setEthFunds(e.ethFunds);
      const ftETH = (1 / e.ethPriceN).toFixed(5);
      setYTtoETH(ftETH);
      setTransactions(e.transactions)
      setMyYT(e.myYT);
      //setStakeBal(e.stakingBalance[e.account]);
      setStakeBal(e.stakeBal);
      setHasStake(e.hasStake);
      setTimeOfStaking(e.timeOfStaking);
      setNumberOfStakers(e.numberOfStakers);
      //setStakingBalance(e.stakingBalance);
  ...
  // Functions
  const isLoading = () => account == null || contractYT == null || contractYTS == null || tokensSold == null || transactionCount == null || ethFunds == null || YTtoETH == null;
  const SliderValue = () => {
  ...
  const buyTokens = async () => {
    ...
    await contractYTS.buyToken(big, {
      from: account,
      value: inputValue * YTtoETH * 10**18,
  ...
  const stakeTokens = async () => {
    ...
    try {
      await contractYT.approve(contractYTS.address, big,{ from:account, gas:50000});
      
      await contractYTS.stakeTokens(big, {
        from: account,
        gas: 500000 // Gas limit
      });
    } catch (error) {
  ...
  const withdrawTokens = async () => {
    const big = BigInt(inputValue * 10**18);
    try {
      await contractYTS.withdrawTokens(big, {
        from: account,
        gas: 500000 // Gas limit
      });
    } catch (error) {
  ...
  return (
    ...
    <VStack minH='800px' w='full' bg={bg}>
      ...
      <HStack w='md' display={{lg: 'flex', md: 'flex', sm: 'none', base: 'none'}}>
        <Heading size='md'>1YT = 1 USD = {YTtoETH} ETH</Heading>
        <Spacer/>
      ...
      <Button bg='green.400' onClick={ () => setIsOpenS(true) }>Stake TOKENS</Button>
      <Button bg='green.400' onClick={ () => setIsOpenW(true) }>Withdraw Stake</Button>
      <Box h='10px' />
      {/* Desktop */}
      ...
      {/* <HStack w='md' h='50px' bg={hBG} borderRadius={8} display={{lg: 'flex', md: 'flex', sm: 'none', base: 'none'}}>
        <Box w='15px' />
        <Text>My StakeBalance: {stakeBal}</Text>
      </HStack>
      <HStack w='full' display={{lg: 'none', md: 'none', sm: 'flex', base: 'flex'}}>
        <Box w='10px'/>
        <HStack w='full' h='50px' bg={hBG} borderRadius={8}>
          <Box w='15px' />
          <Text fontSize='xs'>My StakeBalance: {stakeBal}</Text>
        </HStack>
        <Box w='10px'/>
      </HStack> */}
      ...
      <HStack w='md' h='50px' bg={hBG} borderRadius={8} display={{lg: 'flex', md: 'flex', sm: 'none', base: 'none'}}>
        <Box w='15px' />
        <Text>My Address: {account} YT</Text>
      </HStack>
      {/* Mobile */}
      <HStack w='full' display={{lg: 'none', md: 'none', sm: 'flex', base: 'flex'}}>
        <Box w='10px'/>
      ...
      {/* <HStack w='md' h='50px' bg={hBG} borderRadius={8} display={{lg: 'flex', md: 'flex', sm: 'none', base: 'none'}}>
        <Box w='15px' />
        <Text>Last Stake Date: {timeOfStaking} YT</Text>
      </HStack>
      <HStack w='full' display={{lg: 'none', md: 'none', sm: 'flex', base: 'flex'}}>
        <Box w='10px'/>
        <HStack w='full' h='50px' bg={hBG} borderRadius={8}>
          <Box w='15px' />
          <Text fontSize='xs'>Last Stake Date: {timeOfStaking}</Text>
        </HStack>
        <Box w='10px'/>
      </HStack> */}
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Buy YT</ModalHeader>
          <ModalCloseButton/>
          <ModalBody>
            <Input
            placeholder='1 YT'
      ...
      <Modal isOpen={isOpenS} onClose={onCloseS}>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button bg='green.400' onClick={stakeTokens}>Add</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>

      <Modal isOpen={isOpenW} onClose={onCloseW}>
        <ModalOverlay />
        <ModalContent>
          ...
          <ModalFooter>
            <Button bg='green.400' onClick={withdrawTokens}>Withdraw Rewards</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
    </VStack>
...
const HeaderYT = () => {
  return (
    <VStack>
       <Head>
        <title>Yann Token </title>
        <meta name="description" content="YannToken" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <Box h='5px'/>
      <HStack w='full'>
        <Spacer />
        <Heading size='2xl'>Yann Token </Heading>
        <Spacer />
      </HStack>
      <Box h='10px'/>
    </VStack>
